...
# n() : dplyr 패키지 안에 있는 함수, 데이터의 빈도수를 찾을 때 사용
...
# exam DataFrame에서 race/ethnicity로 그룹화하고, 그룹화한 데이터의 빈도수(Count) / reading score의 평균 값(reading_avg)
# / reading score의 중간 값(백분위 50%/reading_med)
exam %>% group_by(`race/ethnicity`) %>% summarise(count=n(),reading_avg = mean(`reading score`),reading_med = median(`reading score`))



# slice() : index에 의한 행 선택, : 로 원하는 부분 지정 가능.
# 특정한 행을 선택하거나, 제거 가능
# 양수(+) : 해당 행 선택
# 음수(-) : 해당 행 제거
...
exam %>% group_by(`gender`,`race/ethnicity`) %>% 
  slice(6:10) %>% summarise(math_max=max(`math score`),reading_min=min(`reading score`),writing_mean=mean(`writing score`))
...
# 성별으로 내림 차순, group 오름 차순
exam %>% arrange(desc(gender),`race/ethnicity`)
...
# .keep_all : 지정한 열 뿐만 아니라, 모든 열을 보여줌.
